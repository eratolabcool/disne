...
export default function Feature1({ section }: { section: SectionType }) {
  ...
  return (
    <section id={section.name} className="py-16 mt-20">
      <div className="container">
        <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-16">
          ...
          <div className="flex flex-col lg:text-left">
            {section.title && (
              <h2 className="mb-6 text-pretty text-3xl font-bold lg:text-4xl text-black">
                {section.title}
              </h2>
            )}
            {section.description && (
              <p className="mb-8 max-w-xl text-black lg:max-w-none lg:text-lg">
                {section.description}
              </p>
            )}
            <ul className="flex flex-col justify-center gap-y-8">
              {section.items?.map((item, i) => (
                <li key={i} className="flex">
                  ...
                  <div>
                    <div className="mb-3 h-5 text-sm font-semibold text-black md:text-base">
                      {item.title}
                    </div>
