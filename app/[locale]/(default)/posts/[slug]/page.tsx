...
export async function generateMetadata({
  ...
  const baseUrl = process.env.NEXT_PUBLIC_WEB_URL || "https://disneysolitaire.net";
  const canonicalUrl = locale === "en" ? `${baseUrl}/posts/${slug}` : `${baseUrl}/${locale}/posts/${slug}`;

  return {
    title: `${post?.title} | Disney Solitaire Guide`,
    description: post?.description,
    alternates: {
    ...
    openGraph: {
      ...
      images: [
        {
          url: post?.cover_url || "/imgs/maxresdefault.jpg",
          width: 1200,
          height: 630,
          alt: post?.title,
        },
    ...
    twitter: {
      ...
      title: post?.title,
      description: post?.description,
      images: [post?.cover_url || "/imgs/maxresdefault.jpg"],
    },
  };
}
